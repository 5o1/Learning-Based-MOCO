...
class Exp1_one_unet(nn.Module):
    ...
    def __init__(self, in_channels : int = 4 , out_channels : int = 4 , depth : int = 4, top_channels : int = 64, dtype = torch.float, crop_res : bool = True, n_input = 2) -> None:
        ...
        #     nn.Conv2d(64, out_channels, 1)
        # )
        self.unet = Unet(in_channels = in_channels, out_channels = out_channels, depth = depth, top_channels = top_channels, dtype = dtype, crop_res = crop_res)
