...
#     def __init__(self, in_channels: int, out_channels: int, kernel: int = 3, n: int = 2, dtype = torch.float32, padding = 0, norm_layer: nn.Module = None, act_layer : nn.Module = nn.ReLU) -> None:
#         super().__init__()
...
#         self.blks = nn.Sequential([*[nn.Sequential(
#             nn.Conv2d(in_channels if i == 0 else out_channels, out_channels, kernel_size=kernel, padding=padding, dtype=dtype),
